...
*/
		...
		for (int njets=0; njets<nGenJet; njets++){  // jet loop
			njet->Fill(njets);
			jetpt->Fill(GenJet_pt[njets]);
			jeteta->Fill(GenJet_eta[njets]);
			jetphi->Fill(GenJet_phi[njets]);
			if (njets==0){
				jet1pt->Fill(GenJet_pt[njets]);
				jet1eta->Fill(GenJet_eta[njets]);
				jet1phi->Fill(GenJet_phi[njets]); 
				}
			if (njets==1){
				jet2pt->Fill(GenJet_pt[njets]);
                                jet2eta->Fill(GenJet_eta[njets]);
                                jet2phi->Fill(GenJet_phi[njets]);
				}
			if (njets==2){
                                jet3pt->Fill(GenJet_pt[njets]);
                                jet3eta->Fill(GenJet_eta[njets]);
                                jet3phi->Fill(GenJet_phi[njets]);
                                }
			}

		for (int igenpart=0; igenpart<nGenPart; igenpart++){  // gen particle loop
			genpartpt->Fill(GenPart_pt[igenpart]);
			genparteta->Fill(GenPart_eta[igenpart]);
			genpartphi->Fill(GenPart_phi[igenpart]);
			}
		
		for (int igenleptons=0;igenleptons<nGenDressedLepton; igenleptons++){ // gendressedlepton loop
			genleptonpt->Fill(GenDressedLepton_pt[igenleptons]);
			genleptoneta->Fill(GenDressedLepton_eta[igenleptons]);
			genleptonphi->Fill(GenDressedLepton_phi[igenleptons]);
			//if (igenleptons==2){
	...
	//}
		...
		metpt->Fill(GenMET_pt);
		metphi->Fill(GenMET_phi);
}//event loop
	...
	//TFile* of = TFile::Open("dyeej_2NLO3LO_5f_NLO_UNLOPS_nw_output_14122021.root", "RECREATE");
	TFile* of = TFile::Open("dyee012j_5f_NLO_FXFX_nw_output_14122021.root", "RECREATE");
